...
const Reserve = ({ setOpen, hotelId, user }) => {
    ...
    const { data, loading, error } = useFetch(`https://hotel-booking-app-backend-r84q.onrender.com/api/hotels/room/${hotelId}`)
    ...
    const handleClick = async () => {
        try {
            await Promise.all(selectedRooms.map(roomId => {
                const res = axios.put(`https://hotel-booking-app-backend-r84q.onrender.com/api/rooms/availability/${roomId}`, { dates: allDates })
                return res.data
            }))

            await Promise.all(selectedRooms.map(roomId => {
                const resuser = axios.put(`https://hotel-booking-app-backend-r84q.onrender.com/api/rooms/user/${roomId}`, { user: user })
                return resuser.data
            }))
